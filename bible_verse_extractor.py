    ...
    while True:
        ...
        if searchOption == 1:
            ...
            def newSearch(verses, bibleBooks, bibleStats):
                ...
                for a, b, c in zip(foolist[:22], foolist[22:44], foolist[44:]):
    ...
    if len(matches) > 1:
        def whichLanguage(matches):
            ...
            try:
                ...
                translationTitle, code1, code2 = getBibleTextCode(matches[int(choice)-1], languageList, languageLinks)
                languages[languageList[matches[int(choice)-1]]] = (code1, code2)
                translations.append(translationTitle)
                return another(languages, languageList, translations)
            except:
                print("Please choose from the available options \n")
                return whichLanguage(matches)
        ...
        translationTitle, code1, code2 = getBibleTextCode(matches[0], languageList, languageLinks)
        languages[languageList[matches[0]]] = (code1, code2)
        translations.append(translationTitle)
        return another(languages, languageList, translations)
    else:
...
def chooseTranslation(translationList, language):
    ...
    choice = input("Choose a translation: ")
    try:
        if int(choice) not in range(1, len(translationList)+1):
            ...
            return list(translationList.keys())[int(choice)-1], re.search(translationPattern, list(translationList.values())[int(choice)-1]).group(1, 2)
    except:
        print("Please choose from the available options\n")
        return chooseTranslation(translationList, language)
...
def getBibleTextCode(language, languageList, languageLinks):
    ...
    translationTitle, (code1, code2) = chooseTranslation(translationList, languageName)
    return translationTitle, code1, code2
...
h = [{v: k for k, v in languageList.items()}[language] for language in languages.keys()]

for l in languages.keys():
    print("Processing data in {}".format(h[list(languages.keys()).index(l)]))
    verseCodes = [(bibleBooks[verse[0].title()], verse[1], verse[2]) for verse in verses]
    for verse in verseCodes:
        print('    ' + data[verseCodes.index(verse)][0])
        correctLink = 'http://www.bible.com/bible/{}/{}.{}.{}'.format(languages[l][0], verse[0], verse[1], languages[l][1])
        chapterSoup = getPage(correctLink)
        actualLink = chapterSoup.select_one('link[rel="canonical"]').get('href')
        if correctLink.split('.')[-3][-3:] != actualLink.split('.')[-3][-3:]:
            print('There is no translation for {} in {}.'.format({v: k for k, v in bibleBooks.items()}[verse[0]], h[list(languages.keys()).index(l)] + ": " + translations[list(languages.keys()).index(l)])) 
            data[verseCodes.index(verse)].append('')
            continue
        spans = chapterSoup.select('span[class="content"]')
        fullVerse = list()
        for span in spans:
            try:
                if span.parent['class'][1] == 'v{}'.format(verse[2]) and span.getText() != ' ':
...
headers = ['verse'] + [h[i] + ": " + translations[i] for i in range(len(languages))]
